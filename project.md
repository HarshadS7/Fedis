...
# Fedis (IsoVault): Bilateral Underwriting & Insurance for the Agentic Web

## 1. Executive Summary

### Current frontend direction (2026-09-19)

The frontend is presented as a public-facing editorial landing page for Fedis, inspired by the quiet, product-led storytelling of trysmalltalk.co. The risk terminal remains the primary proof surface, with live/mock vault data embedded in the page. The visual system uses warm paper, near-black product surfaces, a single terracotta accent, thin rules, generous whitespace, and no gradients.
...
**Fedis** is a decentralized, bilateral underwriting and insurance clearinghouse built natively for **Monad**. By leveraging Monad's optimistic parallel execution and MonadDB, Fedis creates isolated insurance vaults and dynamic credit scores that scale to thousands of simultaneous agent-to-agent transactions without state contention. 

---

## 2. The Core Protocol Mechanics
Fedis operates on two sides of the autonomous transaction:
...
Fedis is explicitly designed to exploit Monad’s parallel EVM. 
...
* This allows Fedis to clear thousands of policies, payments, and disputes in a single **600ms finality block**, creating the high-frequency liability layer required for machine-to-machine commerce.

---

## 4. Revenue & Monetization Model
Fedis does not rely on taking the LP's capital. The protocol generates revenue via a three-pillar model:

1. **The Origination Fee (The Stripe Model):** Fedis automatically sweeps a 10% take-rate on all insurance premiums paid by the user. LPs keep the other 90%.
2. **Idle Yield (The Bank Model):** Millions in USDC sit in Agent Vaults waiting for claims. Fedis routes this idle capital into blue-chip Monad lending markets, generating a baseline APY. The protocol takes a 20% performance fee on this generated yield.
3. **The Risk Oracle (The B2B Model):** Because Fedis manages the user CIBIL scores and Agent failure rates, it holds the most valuable proprietary data in the agent economy. Other dApps and x402 facilitators pay micro-fees to query the Fedis API for risk assessments before authorizing transactions.
...
* **The Flow:** 
  ...
  3. The user/Agent A pays the task cost + the Fedis premium.
...
> "Visa covers fraud, but they don't cover AI hallucinations, and their 30-cent flat fees destroy the x402 micro-payment economy. The Agentic Web has identity via ERC-8004, but it lacks a liability layer. We built Fedis: the bilateral underwriting clearinghouse for machine commerce. We price risk dynamically based on an agent's historical accuracy and the user's on-chain CIBIL score. By strictly isolating agent vaults, we leverage Monad’s parallel EVM to settle thousands of micro-insurance policies and slashes simultaneously in 600ms. We didn't just build DeFi; we built the missing financial infrastructure for the autonomous economy."
